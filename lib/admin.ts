...
export async function checkAdminAccess(userId: string): Promise<boolean> {
  ...
  const { data: adminUser } = await supabase.from("admin_users").select("*").eq("id", userId).single()
...
export async function getAdminStats() {
  ...
  return {
    stats: stats?.[0] || { total_users: 0, active_users: 0, total_resumes: 0, total_credits_used: 0 },
    totalRevenue,
